...
def correspondingFiles(inputDX, inputPDB):
    ## iterates through inputDX and inputPDB, removing any files that do not have a corresponding dx or pdb
    newDX, newPDB = [], []
    for i in inputDX:
        ind0, ind1 = i.rfind('/'), i.rfind('.pqr')
        dx = i[ind0 + 1: ind1]
        matching = [s for s in inputPDB if dx in s]
        if len(matching) == 1:
            newDX.append(i)
            newPDB.append(matching[0])
    return(newDX, newPDB)
...
def to_csv(results):
    pathlib.Path('blender_output/').mkdir(parents=True, exist_ok=True)
    for i in results:
        id, df = i[0], i[1]
        ind = id.rfind('/')
        id_ = id[ind + 1:-3]
        outName = 'blender_output/%s.csv' %(id_)
        df.to_csv(outName, index=False)

def alignFiles(folderDX, folderPDB):
    pathDX = "%s/*.dx" % (''.join(glob.glob(folderDX)))         # path to *.dx folder
    input_filesDX = sorted(glob.glob(pathDX))                   # input *.dx files
    pathPDB = "%s/*.pdb" % (''.join(glob.glob(folderPDB)))         # path to *.dx folder
    input_filesPDB = sorted(glob.glob(pathPDB))                   # input *.dx files
    dx, pdb = correspondingFiles(input_filesDX, input_filesPDB)   # remove files which do not have corresponding *.pdb or *.dx
    return(dx, pdb)
...
def parse_args():
    ...
    parser.add_argument('-pdb', '--pdbfiles', action='store', help='folder of *.pdb files', type=str)
...
if __name__ == '__main__':
    ...
    dxInput, pdbInput = alignFiles(args.input, args.pdbfiles)
    results = processDX(dxInput, args.analysis, args.cores, False)
